...
export default Vue.extend({
  ...
  async mounted() {
    this.loaded = false;
    this.content = await TranslationClient.getImplementationModelMarkdown("adressenregister");
    this.loaded = true;
  ...
  computed: {
    sidebar(): { title: string; urls: { title: string; url: string }[] } | undefined {
      const sidebar = this.$l("implementationmodel.registries.sidebar");
      if (!sidebar) return undefined;
      return sidebar as { title: string; urls: { title: string; url: string }[] };
    },
  },
});
</script>