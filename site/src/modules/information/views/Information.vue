              ...
              <vl-grid mod-stacked>
                ...
                <vl-column v-if="sidebar.urls > 0" width="3" push="1" push-m="reset" width-s="12">
                  <aside v-vl-sticky="{ top: '120px', left: '0' }">
                    <vl-side-navigation :title="$l(`information.sidebar.title`)">
...
export default Vue.extend({
  ...
  async mounted() {
    this.loaded = false;
    this.header = await TranslationClient.getPageMarkdownHeader("nl", "information");
    this.content = await TranslationClient.getPageMarkdownContent("nl", "information");
    this.loaded = true;
  ...
  computed: {
    spotlightItems(): { title: string; text: string; subtitle: string; url: string }[] {
      const items = this.$l("information.spotlightItems");
    ...
    sidebar(): { title: string; urls: {title:string, url:string}[] } | undefined {
      const items = this.$l("information.sidebar");
