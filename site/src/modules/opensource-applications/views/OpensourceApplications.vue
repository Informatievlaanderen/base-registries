              ...
              <vl-grid mod-stacked>
                ...
                <vl-column v-if="sidebar.urls > 0" width="3" push="1" push-m="reset" width-s="12">
                  <aside v-vl-sticky="{ top: '120px', left: '0' }">
                    <vl-side-navigation :title="$l(`opensourceApplications.sidebar.title`)">
...
export default Vue.extend({
  created() {
    const redirectUrl = this.$l("opensourceApplications.redirect.url");
    if(redirectUrl) {
  ...
  async mounted() {
    this.loaded = false;
    this.repos = await TranslationClient.getGithubRepos("application") || [] as any[];
    this.header = await TranslationClient.getPageMarkdownHeader("nl", "opensource-applications");
    this.content = await TranslationClient.getPageMarkdownContent("nl", "opensource-applications");
    this.loaded = true;
  ...
  computed: {
    sidebar(): { title: string; urls: {title:string, url:string}[] } | undefined {
      const items = this.$l("opensourceApplications.sidebar");
