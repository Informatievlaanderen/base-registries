              ...
              <vl-grid mod-stacked>
                ...
                <vl-column v-if="sidebar.urls > 0" width="3" push="1" push-m="reset" width-s="12">
                  <aside v-vl-sticky="{ top: '120px', left: '0' }">
                    <vl-side-navigation :title="$l(`registries.publicservice.sidebar.title`)">
...
export default Vue.extend({
  created() {
    const redirectUrl = this.$l("registries.publicservice.redirect.url");
    if(redirectUrl) {
  ...
  async mounted() {
    this.loaded = false;
    this.header = await TranslationClient.getPageMarkdownHeader("nl", "registries/publicservice");
    this.content = await TranslationClient.getPageMarkdownContent("nl", "registries/publicservice");
    this.loaded = true;
  ...
  computed: {
    sidebar(): { title: string; urls: {title:string, url:string}[] } | undefined {
      const items = this.$l("registries.publicservice.sidebar");
