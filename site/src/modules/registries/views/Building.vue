              ...
              <vl-grid mod-stacked>
                ...
                <vl-column v-if="sidebar.urls > 0" width="3" push="1" push-m="reset" width-s="12">
                  <aside v-vl-sticky="{ top: '120px', left: '0' }">
                    <vl-side-navigation :title="$l(`registries.building.sidebar.title`)">
...
export default Vue.extend({
  created() {
    const redirectUrl = this.$l("registries.building.redirect.url");
    if(redirectUrl) {
  ...
  async mounted() {
    this.loaded = false;
    this.header = await TranslationClient.getPageMarkdownHeader("nl", "registries/building");
    this.content = await TranslationClient.getPageMarkdownContent("nl", "registries/building");
    this.loaded = true;
  ...
  computed: {
    sidebar(): { title: string; urls: {title:string, url:string}[] } | undefined {
      const items = this.$l("registries.building.sidebar");
