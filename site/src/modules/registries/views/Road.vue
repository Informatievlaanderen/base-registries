              ...
              <vl-grid mod-stacked>
                ...
                <vl-column v-if="sidebar.urls > 0" width="3" push="1" push-m="reset" width-s="12">
                  <aside v-vl-sticky="{ top: '120px', left: '0' }">
                    <vl-side-navigation :title="$l(`registries.road.sidebar.title`)">
...
export default Vue.extend({
  ...
  async mounted() {
    this.loaded = false;
    this.header = await TranslationClient.getPageMarkdownHeader("nl", "registries/road");
    this.content = await TranslationClient.getPageMarkdownContent("nl", "registries/road");
    this.loaded = true;
  ...
  computed: {
    sidebar(): { title: string; urls: {title:string, url:string}[] } | undefined {
      const items = this.$l("registries.road.sidebar");
