              ...
              <vl-grid mod-stacked>
                <vl-column v-if="!loaded && !error" width="12">
                  <div v-vl-align:center>
                    <vl-loader message="De opensource applicaties worden opgevraagd" />
                  </div>
                </vl-column>

                <vl-column v-if="error" width="12">
                  <vl-alert
                    title="Opensource applicaties ophalen mislukt"
                    content="Er is iets fout gelopen tijdens het ophalen van de opensource applicaties. Probeer later opnieuw."
                    mod-error />
                </vl-column>

                <vl-column v-if="loaded" width="12">
                  <vl-typography>
                    <p>Momenteel beheren we <strong>{{ repositories.length }}</strong> opensource applicaties.</p>
                    <p>Deze opensource applicaties komen rechtstreeks van onze <a href="https://github.com/Informatievlaanderen/">GitHub pagina</a> en zijn door hun internationale doelgroep enkel beschikbaar in het Engels.</p>
                    <p>Neem een kijkje bij elke applicatie om meer te komen weten over het gebruik en de bijhorende licentie.</p>
                  </vl-typography>
                  ...
                  <repository
                    ...
                    :to="'/opensource-applicaties/' + item.name" />
                </vl-column>
              </vl-grid>
            </vl-column>
          </vl-grid>
...
export default {
  ...
  methods:
  {
    filterRepositories: function (repositories) {
      var filteredRepositories = _.filter(repositories, { 'topics': ['base-registries-application'] });
